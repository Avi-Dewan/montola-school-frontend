                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {progressData.map((item) => (
                            <Link
                                ...
                                className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition block group"
                            >
                                <div className="mb-4">
                                    <h3 className="text-lg font-bold text-gray-900 group-hover:text-primary-600 transition mb-1">{item.chapterTitle}</h3>
                                    {/* Format 'YYYY-MM-DD' if available, generic otherwise */}
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between text-sm">
                                        ...
                                        <span className="font-medium text-gray-900">{Math.round(item.progressPercentage)}%</span>
                                    </div>
                                    ...
                                    <div className="pt-4 flex items-center justify-between">
                                        <span className={`text-xs px-2 py-1 rounded-full ${item.completed ? 'bg-green-100 text-green-700' : 'bg-primary-50 text-primary-700'}`}>
                                            {item.completed ? "Completed" : "In Progress"}
                                        </span>
                                        <span className="text-sm font-medium text-primary-600 group-hover:underline">Resume &rarr;</span>
                                    </div>
                                </div>
                            </Link>
                        ))}
